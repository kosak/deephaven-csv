...
/**
 * Linked list node that holds data for the communication between a {@link DenseStorageWriter} and {@link DenseStorageReader}. All fields are
...
public final class QueueNode {
    /**
     * Represents a slice of packed bytes in the packed buffer. It is the contract
     * of the code that bytes in this slice are immutable.
     */
    ...
    /**
     * Represents a slice of byte[] references in the large array buffer. It is the contract
     * of the code that the byte[] references in this interval (and the bytes they point to) are immutable.
     */
     ...
     */
    public QueueNode(byte[] packedBuffer, int packedBegin, int packedEnd, byte[][] largeArrayBuffer, int largeArrayBegin,
            int largeArrayEnd) {
